...
$params = $gen->requestParam([
  "sub" => ["policy-sub", "username", 3, 16, [], "MIXED", ["-","_","."]]
], $_GET, ["sub"]);
if (!$params) HTTP\Header::badRequest(true, "Bad request target");
// find sub
$sub = "prj-policy-{$params["sub"]}.md";
if (!\file_exists(PRJ_ROOT . "/src/{$sub}")) HTTP\Header::notFound(true);
$sub_content = \file_get_contents(PRJ_ROOT . "/src/{$sub}");
$sub_content = Markdown::defaultTransform($sub_content);
$page_name = "policy";
?>
<!DOCTYPE html>
<html lang="en" dir="ltr" manifest="/site.webmanifest">
  <head>
    <meta charset="utf-8">
    <title><?php echo Data::getLen(\strip_tags($sub_content),16); ?> | <?php echo PRJ_TITLE; ?></title>
  ...
  <body>
    <?php \TymFrontiers\Helper\setup_page('aboutus','base', true, PRJ_HEADER_HEIGHT); ?>
